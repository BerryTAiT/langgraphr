# langgraphr/R/agent.R

<!-- TARGET: langgraphr/R/agent.R -->

> User-facing objects: `lg_connect()` returns an `LgAgent` (assistant with R
> tools). `LgGraph` (returned by `lg_compile`) runs R-authored graphs.
> Both speak the interrupt/resume loop over HTTP.
