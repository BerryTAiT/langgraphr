# langgraphr/inst/server/runtime.py

<!-- TARGET: langgraphr/inst/server/runtime.py -->

> Compiles graph specs and the assistant into REAL LangGraph graphs, then runs
> them. R-backed nodes/tools pause the run with LangGraph `interrupt()`; the R
> client resumes and this module turns the reply into `Command(goto=, update=)`.
