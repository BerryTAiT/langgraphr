...
>
> By default this installs the frozen, tested versions from `uv.lock`
> (reproducible on every machine). Pass `-Latest` to resolve and install the
> NEWEST versions allowed by `pyproject.toml` instead — useful when you want
> the just-released LangGraph engine, at the cost of reproducibility.
