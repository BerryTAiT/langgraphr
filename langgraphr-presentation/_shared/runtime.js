...
(function(){
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    // mouse wheel navigation (single-page mode): down = next, up = previous
    let wheelAccum = 0;
    let wheelLockUntil = 0;
    document.addEventListener('wheel', function (e) {
      if (scrollMode || editMode) return;
      if (overview.classList.contains('open')) return;
      let dy = e.deltaY;
      if (e.deltaMode === 1) dy = dy * 16;              // lines -> pixels
      if (e.deltaMode === 2) dy = dy * window.innerHeight; // pages -> pixels
      wheelAccum += dy;
      const now = Date.now();
      if (now < wheelLockUntil || Math.abs(wheelAccum) < 40) return;
